...
IMAGE_DATABASE_DIRECTORY = Machine.path.join(BASE_DIRECTORY, "Facial-Emotion")
print(Copyright)
...
for root, dirs, files in Machine.walk(IMAGE_DATABASE_DIRECTORY):
      for file in files:
            if file.endswith("jpg") or file.endswith("png"): # Prefered (.png)
                  ...
                  print(image_array)
                  face = faces_cascades.detectMultiScale(image_array, minNeighbors=5)
                  for (x, y, w, h) in face:
...
with open("labels_emo.pickle", "wb") as file:
